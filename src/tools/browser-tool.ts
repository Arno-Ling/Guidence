...
export class BrowserAutomationTool extends BaseTool {
  ...
  private async handleNavigate(params: Record<string, unknown>): Promise<Result<unknown>> {
  ...
  private async handleScreenshot(params: Record<string, unknown>): Promise<Result<unknown>> {
    await this.ensureBrowser();
  ...
  private async handleExtract(params: Record<string, unknown>): Promise<Result<unknown>> {
    await this.ensureBrowser();
    
    const selector = params.selector as string;
    const extractType = (params.extractType as string) || 'text';
        ...
        content = await Promise.all(
          elements.map(async (el) => {
            switch (extractType) {
              ...
              case 'attribute':
                const attrName = params.attribute as string || 'href';
  ...
  private async handleFillForm(params: Record<string, unknown>): Promise<Result<unknown>> {
    await this.ensureBrowser();
  ...
  private async handleClick(params: Record<string, unknown>): Promise<Result<unknown>> {
    await this.ensureBrowser();
  ...
  private async handleScroll(params: Record<string, unknown>): Promise<Result<unknown>> {
    await this.ensureBrowser();
  ...
  private async handleWait(params: Record<string, unknown>): Promise<Result<unknown>> {
    await this.ensureBrowser();
  ...
  private async handleClose(params: Record<string, unknown>): Promise<Result<unknown>> {
