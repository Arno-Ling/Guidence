...
// 会话存储
const sessions: Map<string, { messages: any[] }> = new Map();
...
app.post('/api/search', async (req, res) => {
  try {
    ...
    const response = await fetch(
      `https://restapi.amap.com/v5/place/text?key=${config.amapApiKey}&keywords=${encodeURIComponent(keyword)}&region=全国&page_size=10`
    );
