...
// 常用软件定义库（增强版）
export const SOFTWARE_REGISTRY: Record<string, SoftwareDefinition> = {
  'gaode_web': {
    name: '高德地图(网页版)',
    type: 'web',
    detectCommand: '',
...
export class SoftwareAutomator {
  ...
  private humanLikeDelay: { min: number; max: number };

  constructor(options?: { humanLikeDelay?: { min: number; max: number } }) {
   ...
   * 检测软件是否安装
   */
  async detectSoftware(softwareId: string): Promise<boolean> {
   ...
   * 打开软件并直接搜索（更高效）
   */
  async openAndSearch(softwareId: string, keyword: string): Promise<AutomationResult> {
    const software = SOFTWARE_REGISTRY[softwareId];
    if (!software) {
   ...
   * 打开软件
   */
  async launchSoftware(softwareId: string): Promise<AutomationResult> {
    const software = SOFTWARE_REGISTRY[softwareId];
    if (!software) {
      return { success: false, error: `未知软件: ${softwareId}` };
    }
   ...
   * 关闭弹窗
   */
  async closePopups(softwareId: string): Promise<void> {
    const page = this.pages.get(softwareId);
    const software = SOFTWARE_REGISTRY[softwareId];
    if (!page || !software?.popupCloseSelectors) return;
   ...
   * 在网页中搜索
   */
  async searchOnWebsite(softwareId: string, keyword: string): Promise<AutomationResult> {
    const page = this.pages.get(softwareId);
    const software = SOFTWARE_REGISTRY[softwareId];
    if (!page) {
   ...
   * 读取页面内容
   */
  async readPageContent(softwareId: string): Promise<AutomationResult> {
    const page = this.pages.get(softwareId);
    if (!page) {
      return { success: false, error: `软件未打开: ${softwareId}` };
    }
   ...
   * 截图
   */
  async takeScreenshot(softwareId: string): Promise<AutomationResult> {
    const page = this.pages.get(softwareId);
    if (!page) {
      return { success: false, error: `软件未打开: ${softwareId}` };
    }
   ...
   * 点击元素
   */
  async clickElement(softwareId: string, selector: string): Promise<AutomationResult> {
    const page = this.pages.get(softwareId);
    if (!page) {
      return { success: false, error: `软件未打开: ${softwareId}` };
    }
   ...
   * 关闭软件
   */
  async closeSoftware(softwareId: string): Promise<void> {
    const page = this.pages.get(softwareId);
    if (page) {
      await page.close();
