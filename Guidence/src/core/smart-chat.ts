...
export interface SmartChatConfig {
  maxHistoryTurns: number;      // 最大保留对话轮数
  systemPromptTemplate: string; // 系统提示词模板
  dataDir: string;              // 数据存储目录
}

export class SmartChatManager {
  private userPrefs: UserPreferencesStore;
...
请用中文回复，保持自然流畅。`;
   ...
   * 从用户消息中提取信息并更新记忆
   */
  private extractAndUpdateMemory(sessionId: string, userId: string, content: string): void {
    // 提取地点
    const placePatterns = [
    ...
    // 提取住宿偏好
    const hotelPatterns = [
      { pattern: /(?:快捷|经济|便宜)/, value: '经济型' as const },
      { pattern: /(?:舒适|中档|三星)/, value: '舒适型' as const },
      { pattern: /(?:高档|四星|品质)/, value: '高档型' as const },
      { pattern: /(?:豪华|五星|奢侈)/, value: '豪华型' as const }
    ];
   ...
   * 获取会话记忆
   */
  getSessionMemory(sessionId: string, userId: string) {
