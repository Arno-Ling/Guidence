...
export class Gateway extends EventEmitter {
    ...
    // ==================== 任务管理 ====================
    async createTask(title, input, options = {}) {
        const task = {
            id: uuidv4(),
    ...
    // ==================== 消息处理 ====================
    async processUserMessage(userInput, userId, sessionId, context) {
        try {
            ...
            // Create task
            const task = await this.createTask(`User: ${userInput.substring(0, 50)}...`, { userInput, context }, { conversationId: session.id });
            ...
            // Get response
            const response = result.data.output?.response || 'Task completed';
